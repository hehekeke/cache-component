...
func (e *TypeError) Error() string {
	return fmt.Sprintf("type not match, field:%s, got type:%v, expected type:%v", e.Field, e.SourceType, e.TargetType)
}
...
func (d *decodeState) unmarshal(v interface{}) (err error) {
	defer func() {
		if r := recover(); r != nil {
			...
			err, ok = r.(error)
			if !ok {
				err = fmt.Errorf("unmarshal encounter unexpected err: %v", r)
			}
		}
	}()
	...
	//Due to mcpack soft delete issue, do not check pack's length
	//Detail see: http://newicafe.baidu.com/issue/baidu-gdp-gdp1-117/show?from=sidePanel
	/*
		if d.off != len(d.data) {
...
func (d *decodeState) value(v reflect.Value) {
	//when data is empty, return []byte("")
	if len(d.data) == 0 {
		d.data = []byte("")
		return
	}
	...
	if v.Kind() == reflect.Interface && v.NumMethod() == 0 {
		...
		case MCPACKV2_STRING:
			v.Set(reflect.ValueOf(d.stringInterface()))
		case MCPACKV2_SHORT_STRING:
			v.Set(reflect.ValueOf(d.shortStringInterface()))
		case MCPACKV2_BINARY:
			v.Set(reflect.ValueOf(d.binaryInterface()))
		case MCPACKV2_SHORT_BINARY:
			v.Set(reflect.ValueOf(d.shortBinaryInterface()))
		case MCPACKV2_INT8:
			v.Set(reflect.ValueOf(d.int8Interface()))
		case MCPACKV2_INT16:
			v.Set(reflect.ValueOf(d.int16Interface()))
		case MCPACKV2_INT32:
			v.Set(reflect.ValueOf(d.int32Interface()))
		case MCPACKV2_INT64:
			v.Set(reflect.ValueOf(d.int64Interface()))
		case MCPACKV2_UINT8:
			v.Set(reflect.ValueOf(d.uint8Interface()))
		case MCPACKV2_UINT16:
			v.Set(reflect.ValueOf(d.uint16Interface()))
		case MCPACKV2_UINT32:
			v.Set(reflect.ValueOf(d.uint32Interface()))
		case MCPACKV2_UINT64:
			v.Set(reflect.ValueOf(d.uint64Interface()))
		case MCPACKV2_BOOL:
			v.Set(reflect.ValueOf(d.boolInterface()))
		case MCPACKV2_FLOAT:
			v.Set(reflect.ValueOf(d.floatInterface()))
		case MCPACKV2_DOUBLE:
			v.Set(reflect.ValueOf(d.doubleInterface()))
		case MCPACKV2_NULL:
			d.nullInterface()
...
func setInt(key []byte, number int64, v reflect.Value) {
	switch v.Kind() {
	...
	default:
		panic(&TypeError{key, reflect.TypeOf(number), v.Type()})
	}
}

func setUInt(key []byte, number uint64, v reflect.Value) {
	switch v.Kind() {
	case reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64, reflect.Int:
		v.SetInt(int64(number))
	case reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uint:
		v.SetUint(number)
	default:
		panic(&TypeError{key, reflect.TypeOf(number), v.Type()})
